...
if not st.session_state.auth:
    ...
    if id_user and id_user not in carregar_db():
        with st.form("registro"):
            ...
            alt = st.number_input("ALTURA (m):", 1.5, 2.2, 1.80); peso = st.number_input("PESO (kg):", 50, 130, 75)
            ...
            if st.form_submit_button("🚀 INICIAR CARREIRA"):
                ...
                db[id_user] = {
                    "nome": n, "idade": i, "altura": alt, "peso": peso, "posicao": pos, "overall": 75, "foto": path,
                    "status": "Saudável", "dna": "Iniciante", "habilidades": [],
...
# --- ÁREA INTERNA ---
    ...
    with st.sidebar:
        ...
        # MELHORIA: ARQUÉTIPO CALCULADO
        arq = definir_arquetipo_master(p, REQUISITOS_ESTILOS)
        st.write(f"**ARQUÉTIPO:** {arq}")
        st.write(f"**STATUS:** {p['status']}")
        
        # MELHORIA: TRATAMENTO MÉDICO
        ...
        for m, v in p["maestria"].items():
            if v > 0: st.caption(f"{m}: {v:.1f}%"); st.progress(v/100)
    ...
    with tabs[0]:
        if p["status"] == "Lesionado": st.error("🚑 BLOQUEADO: Você está no Departamento Médico.")
        else:
            ...
            with col_menu:
                ...
                st.divider(); exibir_laboratorio_skills(p, REQUISITOS_SKILLS)
            
            portal = st.session_state.get('portal')
            if portal == "DEF": 
                from setores.defesa.portal import mostrar_sala_defesa
                mostrar_sala_defesa()
            # Adicione elif para MEI e ATQ chamando os respectivos portais...

    with tabs[1]:
        desenhar_personalidade(p["personalidade"])
        st.divider(); st.subheader("📉 Níveis PES")
        c1, c2, c3, c4 = st.columns(4); fixos = p.get("stats_fixos", {})
        ...
        c4.metric("Resistência Lesão", fixos.get("Resistência a lesão", 2))

    with tabs[2]:
        df = pd.DataFrame(p.get("historico_ovr", []))
        if not df.empty: st.plotly_chart(px.line(df, x="idade", y="ovr"), use_container_width=True)

    with tabs[3]: # CONFIGURAÇÕES (EDIÇÃO TOTAL)
        st.subheader("⚙️ Central de Ajustes")
        with st.form("edit"):
            new_n = st.text_input("Nome:", value=p["nome"])
            new_pos = st.selectbox("Posição:", ["CA", "SA", "MAT", "MLD", "MLE", "VOL", "ZC", "LD", "LE", "GOL"], index=0)
            new_a = st.number_input("Altura:", value=p["altura"]); new_p = st.number_input("Peso:", value=p["peso"])
