...
# --- 2. MOTOR DE ESTADOS E URL PARAMS ---
...
# TELA 1: HUB CENTRAL
    ...
    with c2:
        ...
        if st.button("🚀 CHUTE (EM BREVE)"): pass

# SALA 1: DRIBLE (RANDOM SLALOM - RESTAURADO COMPLETO)
    ...
    <div id="f" style="height:480px; width:100%; background:#0D47A1; position:relative; overflow:hidden; border:4px solid #08306B; touch-action:none; border-radius:20px;">
        ...
        <div style="position:absolute; top:5px; width:100%; text-align:center; color:white; font-weight:bold; font-size:14px;">TEMPO: <span id="t_ui">15.0s</span></div>
    </div>
    ...
    <script>
        const p=document.getElementById('p'), f=document.getElementById('f'), go=document.getElementById('go'), jB=document.getElementById('jb'), jS=document.getElementById('js'), ta=document.getElementById('tArr'), gC=document.getElementById('gCont');
        let x=f.offsetWidth/2-12, y=380, time=15.0, run=false, gts=new Set(), jX=0, jY=0, dragD=false, ck=[];
        function build(){
        ...
        jB.ontouchstart=(e)=>{dragD=true; e.preventDefault();};
        window.ontouchmove=(e)=>{ if(!dragD)return; let t=e.touches[0], r=jB.getBoundingClientRect(), dx=t.clientX-(r.left+40), dy=t.clientY-(r.top+40), d=Math.min(Math.sqrt(dx*dx+dy*dy),40), a=Math.atan2(dy,dx); jX=Math.cos(a); jY=Math.sin(a); jS.style.transform=`translate(${jX*(d/40)*25}px, ${jY*(d/40)*25}px)`; };
        window.ontouchend=()=>{dragD=false; jX=0; jY=0; jS.style.transform='translate(0,0)';};
        go.onclick=()=>{ build(); run=true; go.style.display='none'; ta.style.display='block'; upA(); };
        function resetPos(){ x=f.offsetWidth/2-12; y=380; p.style.left=x+'px'; p.style.top=y+'px'; jX=0; jY=0; jS.style.transform='translate(0,0)'; dragD=false; }
        function loopD(){
            if(run && time>0){
                time-=0.016; document.getElementById('t_ui').innerHTML=time.toFixed(1)+'s';
                x+=jX*4.5; y+=jY*4.5;
                ...
                cones.forEach(c=>{ let cr=c.getBoundingClientRect(), pr=p.getBoundingClientRect(); if(!(pr.right<cr.left||pr.left>cr.right||pr.bottom<cr.top||pr.top>cr.bottom)){ resetPos(); } });
                if(gts.size<5){
...
# SALA 2: PASSE MAESTRO (FIXO + 1 MARCADOR + 10 TENTATIVAS)
    ...
    <script>
        ...
        let jX=0, jY=0, hits=0, tries=10, passing=false, o1D=1, curT=0, startP=0, sttdP=false;
        function upT(){ tgts.forEach((t,i)=>{t.className=i===curT?'tgt act':'tgt';}); }
        jB.ontouchstart=(e)=>{this.drP=true; e.preventDefault(); if(!sttdP){sttdP=true; startP=Date.now();}};
        window.ontouchmove=(e)=>{ if(!this.drP)return; let t=e.touches[0], r=jB.getBoundingClientRect(), dx=t.clientX-(r.left+40), dy=t.clientY-(r.top+40), d=Math.min(Math.sqrt(dx*dx+dy*dy),40), a=Math.atan2(dy,dx); jX=Math.cos(a); jY=Math.sin(a); jS.style.transform=`translate(${jX*(d/40)*25}px, ${jY*(d/40)*25}px)`; aim.style.display='block'; aim.style.transform=`rotate(${(a*180/Math.PI)+90}deg)`; };
        window.ontouchend=()=>{this.drP=false; aim.style.display='none';};
        btnA.onclick=()=>{ if(!passing && tries>0){ if(!sttdP){sttdP=true; startP=Date.now();} shootP(); } };
        function shootP(){
            passing=true; tries--; document.getElementById('t_ui').innerHTML=tries; ball.style.display='block'; let bx=p.offsetLeft+8, by=p.offsetTop+8, vx=jX*12, vy=jY*12;
            let animP=setInterval(()=>{
                ...
                if(!(br.right<tr.left||br.left>tr.right||br.bottom<tr.top||br.top>tr.bottom)){ hits++; curT=(curT+1)%3; upT(); endPass(animP); }
                else if(!(br.right<or1.left||br.left>or1.right||br.bottom<or1.top||br.top>or1.bottom)||bx<0||bx>f.offsetWidth||by<0||by>f.offsetHeight){ endPass(animP); }
            }, 20);
        }
        function endPass(a){ clearInterval(a); ball.style.display='none'; passing=false; document.getElementById('h_ui').innerHTML=hits; if(tries<=0||hits>=10) endGP(); }
        function endGP(){ let ft=(Date.now()-startP)/1000; let bn=ft<12?Math.round((12-ft)*400):0; window.parent.location.href=window.parent.location.href.split('?')[0]+"?p_score="+((hits*200)+bn)+"&hits="+hits; }
        function loopP(){
...
# SALA 3: VELOCIDADE (SISTEMA FPS BLINDADO)
elif st.session_state.pagina == 'vel':
    st.markdown("<h2 style='text-align:center;'>⚡ BURST SPRINT</h2>", unsafe_allow_html=True)
    game_html = """
    <div id="f" style="height:480px; width:100%; background:#0D47A1; position:relative; overflow:hidden; border:4px solid #08306B; border-radius:20px; touch-action:none;">
        <div id="p" style="width:24px; height:24px; background:white; border-radius:50%; position:absolute; left:45%; bottom:40px; border:2px solid #333; transition: box-shadow 0.1s, background 0.1s;">⚽</div>
    ...
    <script>
        ...
        let x=f.offsetWidth/2-12, y=410, t=0, run=false, jX=0, jY=0, dragV=false, spdMult=2, startV=0;
        jB.ontouchstart=(e)=>{dragV=true; if(!run){run=true; startV=Date.now();} e.preventDefault();};
        window.ontouchmove=(e)=>{
            if(!dragV)return; let touch=e.touches[0], r=jB.getBoundingClientRect(), dx=touch.clientX-(r.left+40), dy=touch.clientY-(r.top+40);
            ...
            if(limD > 37) { p.style.background='gold'; p.style.boxShadow='0 0 15px gold'; spdMult=8; } 
            else { p.style.background='white'; p.style.boxShadow='none'; spdMult=2; }
        };
        window.ontouchend=()=>{dragV=false; jX=0; jY=0; jS.style.transform='translate(0,0)';};
        function loopV(){
            if(run){
                t=(Date.now()-startV)/1000; document.getElementById('tm').innerHTML=t.toFixed(2)+'s';
                x+=jX*spdMult; y+=jY*spdMult;
                x=Math.max(0,Math.min(x,f.offsetWidth-24)); y=Math.max(0,Math.min(y,f.offsetHeight-24));
                p.style.left=x+'px'; p.style.top=y+'px';
                if(y<10){ window.parent.location.href=window.parent.location.href.split('?')[0]+"?v_time="+t.toFixed(2); run=false; }
            }
