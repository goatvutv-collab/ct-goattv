...
st.set_page_config(page_title="GOAT TV - CT PRO", layout="centered")
st.markdown("""
    <style>
...
# --- SALA 1: DRIBLE (SETAS NO PERCURSO) ---
elif st.session_state.pagina == 'drible':
    st.markdown("<h2 style='text-align:center;'>⚽ CIRCUITO GUIADO</h2>", unsafe_allow_html=True)
    game_html = """
    ...
    <script>
        ...
        const gates = [{x1:15,x2:45,y:300},{x1:40,x2:70,y:150},{x1:65,x2:95,y:280},{x1:35,x2:65,y:420}];
        function build(){
            gates.forEach((gt,i)=>{
...
# --- SALA 3: VELOCIDADE (INVISÍVEL REAL + REVELAÇÃO) ---
    ...
    <div id="f" style="height:480px; width:100%; background:#0D47A1; position:relative; overflow:hidden; border:10px solid #1E3A8A; border-radius:20px; touch-action:none;">
        ...
        <div style="width:100%; height:8px; background:yellow; position:absolute; top:0;"></div>
    ...
    <script>
        ...
        function build(){
            for(let i=0; i<30; i++){
                let w=document.createElement('div'); w.className='wall'; w.style.width='30px'; w.style.height='30px';
                w.style.left=(Math.random()*80+5)+'%'; w.style.top=(Math.random()*350+40)+'px';
            ...
            if(run){
                ...
                walls.forEach(w=>{
                    ...
                    if(!(pr.right<wr.left||pr.left>wr.right||pr.bottom<wr.top||pr.top>wr.bottom)){ 
                        revT=45; walls.forEach(wall=>wall.classList.add('rev'));
...
# --- NOVO MÓDULO: CHUTE (BASEADO NO TOUR) ---
    ...
    <div id="f" style="height:460px; width:100%; background:#1B5E20; position:relative; overflow:hidden; border:4px solid #08306B; touch-action:none; border-radius:20px;">
        <div id="goal" style="width:200px; height:60px; border:4px solid white; border-bottom:none; position:absolute; left:50%; top:20px; transform:translateX(-50%);">
    ...
    <script>
        ...
        function shoot(){
            ...
            let spd=(pwr>40&&pwr<60)?18:25, vx=jX*spd, vy=jY*spd;
            let anim=setInterval(()=>{
                ...
                if(by<20||bx<0||bx>document.getElementById('f').offsetWidth) endS(anim);
            }, 20);
        }
        ...
        function loop(){ if(pAct && pwr<100){ pwr+=4; pb.style.height=pwr+'%'; } requestAnimationFrame(loop); }
        loop();
    </script>
    """
    components.html(game_html, height=500)
