...
if st.session_state.pagina == 'hub':
    ...
    with c2:
        ...
        if st.button("🔄 REINICIAR CT"): st.rerun()

# SALA 1: DRIBLE (RANDOM SLALOM + COLISÃO PRECISA)
elif st.session_state.pagina == 'drible':
    st.markdown("<h2 style='text-align:center;'>⚽ SLALOM DE PRECISÃO</h2>", unsafe_allow_html=True)
    game_html = """
...
# SALA 2: PASSE MAESTRO (1 MARCADOR COM BLOCOS FIXOS + 10 TENTATIVAS)
    ...
    <div id="f" style="height:460px; width:100%; background:#1B5E20; position:relative; overflow:hidden; border:4px solid #08306B; touch-action:none; border-radius:20px;">
        ...
        <div id="bL" style="width:10px; height:40px; background:#444; position:absolute; left:5%; top:140px; border-radius:3px;"></div>
        <div id="bR" style="width:10px; height:40px; background:#444; position:absolute; right:5%; top:140px; border-radius:3px;"></div>
    ...
    <script>
        const p=document.getElementById('p'), f=document.getElementById('f'), ball=document.getElementById('ball'), btnA=document.getElementById('btnA'), jB=document.getElementById('jb'), aim=document.getElementById('aim'), opp=document.getElementById('opp'), tgts=document.querySelectorAll('.tgt');
        let jX=0, jY=0, hits=0, tries=10, pass=false, oD=3.5, curT=0, start=0, sttd=false;
        ...
        function shoot(){
            pass=true; tries--; document.getElementById('t_ui').innerHTML=tries; ball.style.display='block'; let bx=p.offsetLeft+8, by=p.offsetTop+8, vx=jX*13, vy=jY*13;
        ...
        function endB(a){ clearInterval(a); ball.style.display='none'; pass=false; document.getElementById('h_ui').innerHTML=hits; if(tries<=0||hits>=10) endG(); }
        ...
        function loopP(){
            ...
            opp.style.left=(ox+oD)+'px'; requestAnimationFrame(loopP);
        }
        upT(); loopP();
    </script>
    """
...
# SALA 3: VELOCIDADE (FPS TURBO + BARREIRAS INVISÍVEIS)
    ...
    <div id="f" style="height:480px; width:100%; background:#0D47A1; position:relative; overflow:hidden; border:4px solid #08306B; border-radius:20px; touch-action:none;">
        <div id="p" style="width:24px; height:24px; background:white; border-radius:50%; position:absolute; left:45%; bottom:40px; border:2px solid #333; transition:0.1s;">⚽</div>
    ...
    <script>
        ...
        let x=f.offsetWidth/2-12, y=410, run=false, jX=0, jY=0, drag=false, mult=3, start=0, walls=[];
        for(let i=0; i<6; i++){
            let w=document.createElement('div'); w.style.cssText=`position:absolute; background:transparent; width:60px; height:20px; left:${Math.random()*70+5}%; top:${60+i*50}px; border:1px solid rgba(255,255,255,0.05);`;
            gC.appendChild(w); walls.push(w);
        }
        document.getElementById('jb').ontouchstart=(e)=>{drag=true; if(!run){run=true; start=Date.now();} e.preventDefault();};
        window.ontouchmove=(e)=>{
            if(!drag)return; let touch=e.touches[0], r=document.getElementById('jb').getBoundingClientRect(), dx=touch.clientX-(r.left+40), dy=touch.clientY-(r.top+40);
            ...
            if(d > 37){ p.style.background='gold'; p.style.boxShadow='0 0 15px gold'; mult=9; } else { p.style.background='white'; p.style.boxShadow='none'; mult=3; }
        };
        window.ontouchend=()=>{drag=false; jX=0; jY=0;};
        function loopV(){
            if(run){
                let t=(Date.now()-start)/1000; document.getElementById('tm').innerHTML=t.toFixed(2)+'s';
                x+=jX*mult; y+=jY*mult; x=Math.max(0,Math.min(x,f.offsetWidth-24)); y=Math.max(0,Math.min(y,f.offsetHeight-24));
                p.style.left=x+'px'; p.style.top=y+'px';
                walls.forEach(w=>{
                    let wr=w.getBoundingClientRect(), pr=p.getBoundingClientRect();
                    if(!(pr.right<wr.left||pr.left>wr.right||pr.bottom<wr.top||pr.top>wr.bottom)){ w.style.background='rgba(255,0,0,0.5)'; x=f.offsetWidth/2-12; y=410; }
                });
                if(y<10) window.parent.location.href=window.parent.location.href.split('?')[0]+"?v_time="+t.toFixed(2);
            }
...
# --- RELATÓRIOS ---
    ...
    h=st.session_state.get('hits_p',0); st.markdown(f"<div class='stats-box'><h2>PASSE: {st.session_state.pts_p} PTS</h2><p>Acertos: {h}/10</p></div>", unsafe_allow_html=True)
    if st.button("VOLTAR AO HUB"): st.query_params.clear(); st.session_state.pagina = 'hub'; st.rerun()
